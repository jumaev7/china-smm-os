...
import { approveContent } from '@/api/mutations';
...
import { confirmWorkspaceAction } from '@/utils/confirmAction';
...
function mutationOutcomeMessage(error: unknown): string {
  if (error instanceof AppError) {
    ...
    if (
      ...
      // Outcome may be ambiguous — do not claim definitive failure or replay.
      return (
...
/**
 * Phase 3A approve_content execution — Approvals screen only.
...
export function useApproveContent() {
  ...
  const runApprove = useCallback(
    async (attentionId: string, action: OperatorWorkspaceAction) => {
      // Eligibility: backend action_id only — never invent from category/status.
      if (action.action_id !== 'approve_content') return;
      if (!action.enabled) return;
      if (!isMobileMutationAllowed('approve_content')) {
        Alert.alert('Unavailable', 'Approve is not enabled on this build.');
        return;
      }
      if (isOffline) {
        Alert.alert('Offline', 'Approve is unavailable while offline.');
        return;
      }
      if (status !== 'authenticated' || !getAccessToken()) {
        Alert.alert('Session', 'Sign in again before approving.');
        return;
      }
      ...
      if (action.requires_confirmation) {
        const accepted = await confirmWorkspaceAction(action);
        if (!accepted) return;
      }
      ...
      try {
        const result = await approveContent(attentionId);
        // Refetch canonical state — do not synthesize local approved.
        await invalidateOperatorReads(queryClient);
        Alert.alert(
          'Approved',
          result.message?.trim() ||
            'Content approved — client review started where configured.',
        );
      } catch (err) {
        const shouldRefresh =
        ...
        Alert.alert('Could not approve', mutationOutcomeMessage(err));
