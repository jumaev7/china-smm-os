...
import {
  ...
  WeChatProviderHealthItem,
  WeChatProviderType,
} from "@/lib/api";
import { cn } from "@/lib/utils";
import { EmptyState, ErrorState, LoadingState } from "@/components/ui/PageStates";

const PROVIDER_TYPE_LABELS: Record<WeChatProviderType, string> = {
  wecom_api: "WeCom API",
  official_account_api: "Official Account API",
...
export default function WeChatProviderPage() {
  const qc = useQueryClient();
  const [selectedProviderId, setSelectedProviderId] = useState<string | null>(null);
  const [registerName, setRegisterName] = useState("");
  const [registerType, setRegisterType] = useState<WeChatProviderType>("wecom_api");
  ...
  const {
    ...
    queryKey: ["wechat-provider-providers"],
    queryFn: () => wechatProviderApi.listProviders().then((r) => r.data),
  });

  const { data: configurationsData, refetch: refetchConfigurations } = useQuery({
    queryKey: ["wechat-provider-configurations"],
    queryFn: () => wechatProviderApi.listConfigurations().then((r) => r.data),
    enabled: !!providersData,
  });

  const { data: health, refetch: refetchHealth } = useQuery({
    queryKey: ["wechat-provider-health"],
    queryFn: () => wechatProviderApi.health().then((r) => r.data),
    enabled: !!providersData,
  });
  ...
  const testConnectionMut = useMutation({
    mutationFn: (providerId: string) => wechatProviderApi.testConnection(providerId),
    onSuccess: (res) => {
      if (res.data.ok) toast.success(res.data.message);
      else toast.error(res.data.message);
      qc.invalidateQueries({ queryKey: ["wechat-provider-health"] });
      qc.invalidateQueries({ queryKey: ["wechat-provider-configurations"] });
    },
  ...
  const registerMut = useMutation({
    mutationFn: () =>
      wechatProviderApi.registerProvider({
    ...
    onSuccess: (res) => {
      ...
      setRegisterName("");
      qc.invalidateQueries({ queryKey: ["wechat-provider-providers"] });
      qc.invalidateQueries({ queryKey: ["wechat-provider-configurations"] });
      qc.invalidateQueries({ queryKey: ["wechat-provider-health"] });
    },
  ...
  const capabilityMatrix = useMemo(() => {
    return providers.map((p: WeChatProvider) => ({
      id: p.id,
  ...
  if (providersLoading) return <LoadingState title="Loading WeChat Provider…" />;
  if (providersError) {
    return (
      <ErrorState
        title="Failed to load WeChat Provider"
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 max-w-xl">
            Production-grade provider architecture for WeCom, Official Account, and third-party
            connectors. Configuration and health checks only — no message sending or live API calls
            in v1.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Link href="/wechat-sync" className="btn-secondary text-sm">
            WeChat Sync
          </Link>
          <Link href="/wechat" className="btn-secondary text-sm">
            WeChat Center
          </Link>
        ...
        <div className="flex flex-wrap gap-2 items-end">
          ...
          <label className="text-xs text-gray-500 flex flex-col gap-1">
            ...
            <select
              ...
              value={registerType}
              onChange={(e) => setRegisterType(e.target.value as WeChatProviderType)}
            >
        ...
        <div className="card p-4 space-y-3">
          ...
          {providers.length === 0 ? (
            ...
            <ul className="space-y-2">
              {providers.map((p: WeChatProvider) => (
                <li
                  key={p.id}
                  className={cn(
        ...
        <div className="card p-4 space-y-3">
          ...
          {configurations.length === 0 ? (
            ...
            <ul className="space-y-2 max-h-[360px] overflow-y-auto">
              {configurations.map((c: WeChatProviderConfiguration) => (
                <li key={c.id} className="border border-gray-100 rounded-lg p-3 text-xs">
                  ...
                  <p className="text-gray-500 mt-1">
                    Tenant: {c.tenant_id ?? "global demo"}
                  </p>
                  <p className="text-[10px] text-gray-400 mt-1">
                    Last test: {formatDt(c.last_connection_test)} · Created: {formatDt(c.created_at)}
                  </p>
                </li>
              ))}
            </ul>
      ...
      <div className="card p-4 space-y-3">
        ...
        <p className="text-[10px] text-gray-400">
          Message Send is tracked but disabled in v1 — operators send manually from WeChat Center.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="card p-4 space-y-3">
          ...
          {(health?.provider_health ?? []).length === 0 ? (
            ...
            <ul className="space-y-2">
              {(health?.provider_health ?? []).map((item: WeChatProviderHealthItem) => (
                <li key={item.provider_id} className="border border-gray-100 rounded-lg p-3 text-xs">
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">
                    {PROVIDER_TYPE_LABELS[item.provider_type]} · Last test:{" "}
                    {formatDt(item.last_connection_test)}
                  </p>
                </li>
              ))}
            </ul>
        ...
        <div className="card p-4 space-y-3">
          ...
          <p className="text-[10px] text-gray-400 flex items-start gap-1.5">
            ...
            Webhook handlers (inbound_message, contact_update, conversation_update) are registered
            for architecture only — no live processing in v1.
          </p>
        </div>
      </div>
    </div>
