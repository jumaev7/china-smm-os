...
const JOB_STATUS_STYLES: Record<WeChatSyncJobStatus, string> = {
...
export default function WeChatSyncPage() {
  ...
  const {
    ...
    queryKey: ["wechat-sync-accounts"],
    queryFn: () => wechatSyncApi.listAccounts().then((r) => r.data),
  });

  const { data: status } = useQuery({
    queryKey: ["wechat-sync-status"],
    queryFn: () => wechatSyncApi.status().then((r) => r.data),
    enabled: !!accountsData,
  });

  const { data: jobsData, refetch: refetchJobs } = useQuery({
    queryKey: ["wechat-sync-jobs"],
    queryFn: () => wechatSyncApi.listJobs({ limit: 30 }).then((r) => r.data),
    enabled: !!accountsData,
  });
  ...
  const syncContactsMut = useMutation({
    mutationFn: () =>
      wechatSyncApi.syncContacts(
        activeAccountId ? { account_id: activeAccountId } : {},
      ),
    onSuccess: (res) => {
      toast.success(res.data.message || "Contacts synced");
      qc.invalidateQueries({ queryKey: ["wechat-sync-jobs"] });
      qc.invalidateQueries({ queryKey: ["wechat-sync-accounts"] });
      qc.invalidateQueries({ queryKey: ["wechat-sync-status"] });
    },
  ...
  const syncConversationsMut = useMutation({
    mutationFn: () =>
      wechatSyncApi.syncConversations(
        activeAccountId ? { account_id: activeAccountId } : {},
      ),
    onSuccess: (res) => {
  ...
  const testConnectionMut = useMutation({
    mutationFn: (accountId: string) => wechatSyncApi.testConnection(accountId),
    onSuccess: (res) => {
  ...
  if (accountsLoading) return <LoadingState title="Loading WeChat Sync…" />;
  if (accountsError) {
    return (
      <ErrorState
        title="Failed to load WeChat Sync"
      ...
      <div className="card p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2 items-end">
          <label className="text-xs text-gray-500 flex flex-col gap-1">
            ...
            >
              {accounts.map((a: WeChatSyncAccount) => (
        ...
        <p className="text-[10px] text-gray-400">
          Sync imports inbound data into Unified Inbox and Communication Intelligence. Operators
          send messages manually from WeChat Center.
        </p>
      </div>
