...
HEAD = "20260906_marketing_intelligence"
...
async def _run() -> int:
    ...
    record("current_is_mip_head", HEAD in (cur.stdout or ""), (cur.stdout or "")[:200])

    down = _alembic("downgrade", PREV)
    record("downgrade_to_workflow", down.returncode == 0, (down.stderr or down.stdout)[-400:])
