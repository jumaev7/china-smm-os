"""WeChat Sync v1 — account registry and sync jobs (integration-ready, no auto-send)."""
import uuid
...
WECHAT_ACCOUNT_TYPES = ("personal_wechat", "wecom", "official_account")
WECHAT_ACCOUNT_STATUSES = ("not_connected", "connected", "sync_error", "disabled")
WECHAT_SYNC_JOB_STATUSES = ("pending", "running", "completed", "failed")
WECHAT_SYNC_JOB_TYPES = (
    "contacts",
    "conversations",
    "test_connection",
    "scheduled_contacts",
...
WECHAT_SYNC_TRIGGERS = ("manual", "scheduled")


class WeChatSyncAccount(Base):
    __tablename__ = "wechat_sync_accounts"
...
class WeChatSyncJob(Base):
    __tablename__ = "wechat_sync_jobs"
    ...
    account_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("wechat_sync_accounts.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
