"""LinkedIn publisher adapter (mock)."""
...
async def publish(ctx: PublishContext) -> dict:
    post_id = f"mock-li-{secrets.token_hex(6)}"
    return {
        "platform": "linkedin",
        "success": True,
        "mock": True,
        "platform_post_id": post_id,
        "message": f"[Mock] Posted to LinkedIn ({ctx.account_name or 'account'}) for {ctx.company_name}",
