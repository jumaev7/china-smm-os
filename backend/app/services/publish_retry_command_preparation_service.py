...
forensic failure_code of ``retry_command_prepared``. Phase 3C.1C-D is expected
to consume a still-``claimed`` command with ``resulting_attempt_id`` set and
``provider_write_started_at IS NULL``, then cross the write barrier.
